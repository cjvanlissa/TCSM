...
<details>
  <summary>Click for explanation</summary>
The package `psych` contains many functions for exploring data. If you have not yet done so, install the package, then load it and use the `describe()` function to describe the data:
...
*Hint: The function* `cor()` *provides Pearson's correlations. Select variables by name from a data.frame object (like* `data`*) using the following syntax*: `data[, c("each", "variable", "name")]`*.*

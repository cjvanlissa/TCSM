...
Note that, in this case, parallel analysis recommends as many as 6 factors.
...
Now you can analyze the differences between the factor scores for the PCA analysis (take-home exercise 2) and the EFA by plotting them in a series of scatterplots (bivariate) using `ggplot2`. The PCA factor scores are already stored in the dataset `ESSround1-b.csv`.

<details>
  <summary>Click for explanation</summary>

*Make sure to adjust the variable names to the variables names you used.*
...
```{r, eval = FALSE}
cor(df$EFA_Trust1, df$PCA_Trust1, use = "complete.obs")
```
```{r, echo = FALSE, warning = FALSE}
cor(df$trustinstEFA, df$trustinst, use = "complete.obs") 
```
...
```{r}
df_finland <- df[df$cntry == "Finland", ]
```

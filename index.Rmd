...
schedule$Topic <- topics[match(schedule$weeks, sort(unique(schedule$weeks)))]
...
# lubridate::isoweek(as.Date("13-09-2021", format = "%d-%m-%Y"))
DT::datatable(schedule[order(as.Date(schedule$Date, format = "%d-%m-%Y"), decreasing = FALSE), ], rownames = FALSE, options = list(pageLength = nrow(schedule)))
```
...
The course consists of three parts, each of which are examinated with an assignment:
...
The first assignment (deadline: `r schedule$Date[schedule$Activity == "Exams"][1]`) is an assignment in which ) assesses your understanding of the latent variable model. This is a group assignment, made with your learning team. The exam counts towards 25% of your grade.
...
A description of the assignments follows below. For each assignment, every element labeled with a lower case letter is graded pass (0 points), fail (1 point), or excellent (1.5 points). Grades are summed for each assignment, and rescaled from 1-10. The final grade is a weighted average across assignments of the rescaled grades (weights in %). 
...
•	The final assignment also helps you assess of your ability to independently analyse data, which is important to know for your future courses and/or career.
